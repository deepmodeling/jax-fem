...
| Layer time, layers 251+ | **RECONSTRUCTED 2026-07-30 (D-10, inferred)** with one OPEN residual | `derived/layer-schedule.json`: per-layer clock from the JRES tables (dead time 34.4 s = 52 − computed legs active, layer-invariant assumption I1; ridges band rules inferred, I2). Closed loop: computed legs plate scan 17.6 s vs published "~26 s" — **−32 % residual, unresolved**. Two structural readings bracket layers 251+: (A, adopted) constant absolute dead time → bridge ≈ 54-56 s/layer, build ≈ 9.23 h; (B) dead = 52−26 = 26 s with the unmodeled ~8.4 s scaling with active time → bridge ≈ 64 s, build ≈ 9.9 h. NOTE: the "9.4 h build" quoted in earlier ledger arguments (D-05 retraction, C.2, E-emissivity) is an internal estimate, NOT a published number — those arguments are order-of-magnitude and unaffected. Candidate discriminator: frame timestamps in the M31935 thermography .mat files (timing use does not touch the emissivity objection that caused its rejection) — needs user approval to reopen | Medium — this is 60 % of the build |
