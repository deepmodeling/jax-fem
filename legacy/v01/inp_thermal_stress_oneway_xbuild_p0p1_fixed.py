...
def material_cell_state(active_cell, substrate_cell, support_cell, args, cell_temperature=None):
    # State encoding for ParaView:
    ...
    #   5 substrate
    #   6 support
    inactive_code = 1.0 if args.powder_mode == "powder" else 0.0
    state = inactive_code * onp.ones(len(active_cell), dtype=onp.float64)
    state[active_cell] = 2.0
    if cell_temperature is not None and args.liquidus_temperature > args.solidus_temperature:
        ...
        state[mushy] = 3.0
        state[liquid] = 4.0
    state[substrate_cell] = 5.0
    state[support_cell] = 6.0
    return state
...
def main():
    ...
    last_material_state = material_cell_state(last_active_cell, substrate_cell, support_cell, args)
    ...
    for state in step_states:
        ...
        material_state_cell = material_cell_state(active_cell, substrate_cell, support_cell, args, cell_T)
