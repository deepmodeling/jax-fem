...
class TransientThermal(Problem):
    ...
    def get_mass_map(self):
        def heat_capacity(
            ...
            return np.array([rho[0] * cp_eff * (T[0] - T_old[0]) / dt[0] - q_vol + q_front_loss])
...
def update_layers_from_thickness(args, pmin, pmax, build_axis_id):
    ...
    if derived_layers != args.layers:
        print(f"INFO: --layer-thickness overrides --layers: layers {args.layers} -> {derived_layers}")
...
def thermal_material_quads(T_old_quad, active_quad, phase_quad, args, tables):
    ...
    conductivity_quad = np.where(
        is_void,
        k_void,
        np.where(is_powder, k_powder_quad, np.where(is_liquid, k_liquid_quad, np.where(is_mushy, k_mushy, k_solid_quad))),
    )

    # Inactive void material should never accumulate energy as printed material.
    rho_quad = np.where(active_quad > 0.5, rho_quad, np.where(args.powder_mode == "powder", args.rho_powder, rho_void))
    cp_quad = np.where(active_quad > 0.5, cp_quad, np.where(args.powder_mode == "powder", cp_powder_quad, cp_void))
    conductivity_quad = np.where(active_quad > 0.5, conductivity_quad, np.where(args.powder_mode == "powder", k_powder_quad, k_void))

    if args.latent_heat > 0.0 and args.liquidus_temperature > args.solidus_temperature:
        in_mushy = (T_old_quad >= args.solidus_temperature) & (T_old_quad <= args.liquidus_temperature) & (active_quad > 0.5)
...
def main():
    ...
    for state in step_states:
        ...
        active_cell = previous_active | raw_active_cell
        ...
        phase_quad = np.where((active_quad > 0.5) & (phase_quad == STATE_VOID), STATE_POWDER, phase_quad)
        T_old_quad = thermal.fes[0].convert_from_dof_to_quad(T_old)
        rho_quad, cp_quad, conductivity_quad, latent_cp_quad = thermal_material_quads(T_old_quad, active_quad, phase_quad, args, tables)
