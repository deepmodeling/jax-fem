...
#
# 2026-08-04(P1,Fable5 排序):末端由 0.2 延到 2.0。原因是 j2.py 的
# _interpolate_clamped 对 eps_p 轴做端点钳制,越过表末端后
# _plastic_increment_from_curve 走 beyond_root 分支,切线**恰好为 0**——
# 也就是静默进入理想塑性:不报错、不失败,但那一轮的收敛结论与 RS 数值整体作废,
# 事后还查不出来。50 W 探针载荷下越界单元 0/30000(实测 eqp 峰值 0.0233),够不着;
# 220 W 平价功率下热带大得多,会够着。便宜的防呆必须挡在昂贵的运行前面。
#
# 延长是**纯增量**的:build() 逐点独立计算,最小切线又是自低应变向上递推,
# 所以追加高应变格点不会改动 eps_p <= 0.2 的任何已有值。已逐字节核验。
