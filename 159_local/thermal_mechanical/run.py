...
class TransientThermal(Problem):
    def custom_init(self, rho, cp, conductivity, convection_h, ambient):
        self.rho = rho
        self.cp = cp
    ...
    def get_mass_map(self):
        def heat_capacity(T, x, T_old, dt, laser_center, laser_power, beam_radius, source_depth, laser_switch):
            r2 = (x[0] - laser_center[0]) ** 2 + (x[1] - laser_center[1]) ** 2
            z2 = (x[2] - laser_center[2]) ** 2
            q_vol = (
                laser_power
                / (np.pi * beam_radius**2 * source_depth)
                * np.exp(-2.0 * r2 / beam_radius**2)
                * np.exp(-2.0 * z2 / source_depth**2)
                * laser_switch
            )
    ...
    def get_surface_maps(self):
        def top_flux(_T, point, old_T, laser_center, laser_power, beam_radius, laser_switch):
            r2 = (point[0] - laser_center[0]) ** 2 + (point[1] - laser_center[1]) ** 2
            q_laser = (
                2.0
                * laser_power
                / (np.pi * beam_radius**2)
...
def make_box_locations(points, tol_ratio=1e-8):
    ...
    def bottom(point):
        return np.isclose(point[2], pmin[2], atol=atol)

    def top(point):
        return np.isclose(point[2], pmax[2], atol=atol)

    def walls(point):
        return (
            np.isclose(point[0], pmin[0], atol=atol)
            | np.isclose(point[0], pmax[0], atol=atol)
            | np.isclose(point[1], pmin[1], atol=atol)
            | np.isclose(point[1], pmax[1], atol=atol)
        )

    return pmin, pmax, bottom, top, walls


def make_laser_path(pmin, pmax, args):
    axis_id = {"x": 0, "y": 1, "z": 2}[args.scan_axis]
    span = pmax - pmin
    midpoint = 0.5 * (pmin + pmax)
    base = onp.array([midpoint[0], midpoint[1], pmax[2]], dtype=onp.float64)
...
def main():
    ...
    pmin, pmax, bottom, top, walls = make_box_locations(points)
    xspan, yspan, zspan = pmax - pmin
    xy_scale = max(float(xspan), float(yspan), 1e-12)
    beam_radius = args.beam_radius if args.beam_radius > 0 else 0.08 * xy_scale
    source_depth = args.source_depth if args.source_depth > 0 else max(0.5 * beam_radius, 0.1 * max(float(zspan), 1e-12))
    scan_start, scan_end = make_laser_path(pmin, pmax, args)
    ...
    thermal = TransientThermal(
        ...
        additional_info=(args.rho, args.cp, args.conductivity, args.convection_h, args.ambient),
    )
    ...
    print("scan_axis:", args.scan_axis)
