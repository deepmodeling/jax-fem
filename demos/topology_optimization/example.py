...
class Elasticity(Problem):  # 定义具体问题
    ...
    def get_tensor_map(self):
        def stress(u_grad, theta):
            ...
            # u_grad: gradient of displacement field
            # theta: density field
    ...
    def set_params(self, params):
        # Override base class method.
        # 这个函数的作用是将设计变量theta设置到问题中
        full_params = np.ones((self.fe.num_cells, params.shape[1]))
        full_params = full_params.at[self.fe.flex_inds].set(
            params
        )  # 将params中的值设置在 full_params 的flex_inds指定的位置
    ...
    def compute_compliance(self, sol):
        ...
        # Nonson公式是连续介质力学中的概念，用于描述材料变形时面积向量的变换；nonson_scale表示每个面的变换尺度因子，用于计算变形后的面积向量
        u_face = (
        ...
        traction = -jax.vmap(jax.vmap(neumann_fn))(
...
# meshio_mesh.points: coordinates of grid nodes
# meshio_mesh.cells_dict[cell_type]: connectivity of grid nodes (node id in one element)


# Define boundary conditions and values.
def fixed_location(point):
    return np.isclose(point[0], 0.0, atol=1e-5)  # 找到与 0.0 在 1e-5 误差范围内的点
...
fwd_pred = ad_wrapper(
    problem, linear=True, use_petsc=True, use_petsc_adjoint=True
)  # 决定是否可以自动微分的关键一步
...
def output_sol(params, obj_val):
    ...
    save_sol(
        problem.fe,
        np.hstack((sol, np.zeros((len(sol), 1)))),
        vtu_path,
...
numConstraints = 1
optimize(
    problem.fe, rho_ini, optimizationParams, objectiveHandle, consHandle, numConstraints
)
print(
    f"As a reminder, compliance = {J_total(np.ones((len(problem.fe.flex_inds), 1)))} for full material"
)
