...
import meshio
import unittest
from jax_am.fem.jax_fem import Mesh, Plasticity
...
class Test(unittest.TestCase):
    ...
    def test_solve_problem(self):
        ...
        disps = np.load(f"jax_am/fem/tests/{problem_name}/fenicsx/disps.npy")
        ...
        fenicsx_avg_stresses = np.load(f"jax_am/fem/tests/{problem_name}/fenicsx/avg_stresses.npy")
        jax_fem_avg_stresses = avg_stresses[:, 2, 2]
        np.save(f"jax_am/fem/tests/{problem_name}/jax_fem/avg_stresses.npy", jax_fem_avg_stresses)
